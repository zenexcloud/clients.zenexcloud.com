<div data-services-widget-wrapper>
    ...
    <div class="service-widget is-hidden" data-services-widget>
        ...
        <div class="user-general">
                ...
                <div class="user-general__info">
                    ...
                    <div class="user-general__info-box">
                        ...
                        <h4 class="user-general__name text--bold text--danger no-wrap">{$details.name}</h4>
                    </div>
                </div>
                ...
                <div class="user-general__service-mrr">
                    <p class="text--sm text--bold no-wrap">Service MRR:</p>
                    <span class="user-general__service-stat text--light no-wrap is-hidden" data-service-mrr>
                        {if $serviceMRR}
                            {$serviceMRR}
                        {else}
                    ...
                    <div class="popover__loader popover__loader--sm" data-service-mrr-loader>
                        <div class="loader"></div>
                    </div>
                </div>
                {if $clientTotalIncome}
                    <div class="user-general__service-income">
                        <p class="text--sm text--bold no-wrap">Total Income:</p>
                        <span class="user-general__service-stat text--light">{$clientTotalIncome}</span>
                    </div>
                {/if}
                {if $serviceIncome}
                    <div class="user-general__service-income">
                        <p class="text--sm text--bold no-wrap">Service Income:</p>
                        <span class="user-general__service-stat text--light">{$serviceIncome}</span>
                    </div>
                {/if}
                <div class="user-general__service-activity">
                    <p class="text--sm text--bold no-wrap vertical-flex-center">Active Since:
                        {if $serviceActiveSince}
                            <svg class="icon-color" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"
                                data-tooltip data-title="Active for {$serviceActiveSince.diff}.">
                    ...
                    <span class="user-general__service-stat text--light {if !$serviceActiveSince}text--danger{/if}">{if $serviceActiveSince}{$serviceActiveSince.formattedDate}{else}Inactive{/if}</span>
                </div>
                <div class="md-break"></div>
                {if $serviceAssignedDomain}
                    <div class="user-general__service-domain">
                        <a href="{$serviceAssignedDomain.domainLink}">
                            Domain: {$serviceAssignedDomain.domain}
                            <label data-tooltip
                                data-title="{if isset($serviceAssignedDomain.status)}{$serviceAssignedDomain.status}{/if}"
                                class="label label--sm label--ring label--{if $serviceAssignedDomain.status == "Active"}green{else}red{/if}"> </label>
                        </a>
                        <a href="http://{$serviceAssignedDomain.domain}" target="_blank">
                            <svg class="link-svg" width="16" height="16" viewBox="0 0 16 16" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M12.5274 8.2277L12.5273 12.1667C12.5273 12.5203 12.3868 12.8594 12.1368 13.1095C11.8867 13.3595 11.5476 13.5 11.194 13.5H3.83333C3.47971 13.5 3.14057 13.3595 2.89052 13.1095C2.64048 12.8594 2.5 12.5203 2.5 12.1667L2.50011 4.83333C2.50011 4.47971 2.64058 4.14057 2.89063 3.89052C3.14068 3.64048 3.47982 3.5 3.83344 3.5H7.86073"
                                    stroke="#0091EA" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M10.5 1.5H14.5V5.5" stroke="#0091EA" stroke-linecap="round"
                                    stroke-linejoin="round"/>
                                <path d="M7 9L14.5 1.5" stroke="#0091EA" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </a>
                    </div>
                {else}
                    <div class="user-general__service-domain">
                        <span class="text--faded no-wrap">No Registered Domains</span>
                    </div>
                {/if}
                <div class="user-general__service-related dropdown" {if $serviceRelatedServices} data-dropdown="services" data-placement="bottom-end" {/if}>
                    {if $serviceRelatedServices}
                        <a class="dropdown-toggler {if !$serviceRelatedServices}no-services{/if}" data-dropdown-toggle>
                            Related Services:
                            <label class="label label--dot label--lg {if $serviceRelatedServices} label--blue-3 {else} label--red-3{/if}">{if isset($serviceRelatedServices) && $serviceRelatedServices && $serviceRelatedServices|count}{$serviceRelatedServices|count}{else}0{/if}</label>
                            {if $serviceRelatedServices}
                        ...
                        <div class="dropdown-menu user-general__service-dropdown" data-dropdown-menu="services"
                            ...
                            <div class="dropdown-menu__inner" data-filters="currentPeriod">
                                <ul class='tooltip-list dropdown-menu__items'>
                                    {foreach from=$serviceRelatedServices item=service}
                    ...
                    {else}
                        <div class="user-general__service-domain">
                            <span class="text--faded no-wrap">No Related Services</span>
                        </div>
                    {/if}
                </div>
        </div>
