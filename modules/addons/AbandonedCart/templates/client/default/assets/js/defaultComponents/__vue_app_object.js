...
var mgDefauleVueObject = {
            ...
            methods: {
                ...
                loadModal : function(event, targetId, namespace, index, params, addSpinner,postData = []){
                ...
                reloadModalContent: function(event, targetId, namespace, index, params, addSpinner) {
                    ...
                    if($('#mgModalContainer').find('form').length > 0) {
                        ...
                        const dataTable = $(event.target).parents('.vueDatatableTable').first();
                        if (dataTable) {
                             self.massActionIds = collectTableMassActionsData(dataTable.attr('id'));
                            for (let key in self.massActionIds) {
                    ...
                    $.post(self.targetUrl, parsedFormData).done(function (data) {
                        data = data.data;
                        if(data.status === 'success'){
                           self.htmlContent = data.rawData.htmlData;
