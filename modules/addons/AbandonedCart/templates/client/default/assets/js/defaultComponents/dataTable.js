mgJsComponentHandler.addDefaultComponent('mg-datatable', {
    ...
    created: function () {
        ...
        self.additionalData = self.additional_data;
        ...
        {
            this.$root.$on('reloadMgData', this.updateMgData);
        }
    ...
    methods: {
        ...
        updateProjects: function(){
            ...
            resp.done(function(data){
                if (data.data.status === 'error') {
                    ...
                    if(actionData.additional_data)
                    {
                        self.additionalData = actionData.additional_data;
                    }

                    const customParams = data.data.customParams ? data.data.customParams : {};
                    ...
                    mgEventHandler.runCallback('DatatableDataLoaded', self.component_id, {data: data.records, datatable: self,customParams: customParams});
        ...
        loadModal: function(event, targetId, namespace, index, params, addSpinner){
            mgPageControler.vueLoader.massActionIds = collectTableMassActionsData(this.$el.getAttribute('id'));
            let postData = [];
            if (mgPageControler.vueLoader.massActionIds) {
                for (let index in mgPageControler.vueLoader.massActionIds) {

                    if (!mgPageControler.vueLoader.massActionIds.hasOwnProperty(index)) {
                        continue;
                    }
                    postData.push({'name': `massActions[${index}]`, 'value': mgPageControler.vueLoader.massActionIds[index]});
                }
            }
            if (this.additionalData) {
                ...
                params.push({'name':'additional_data','value':this.additionalData});
            }
            mgPageControler.vueLoader.loadModal(event, targetId,
                typeof namespace !== 'undefined' ? namespace : getItemNamespace(targetId),
                typeof index !== 'undefined' ? index : getItemIndex(targetId), params, addSpinner,postData);
        },
