{foreach from=$menu item=item}
    <li id="{if $item.css_id}{$item.css_id}{else}Secondary_Navbar-Account{/if}" class="{if $item.children}dropdown{/if}{if $item.css_class} {$item.css_class}{/if}">
        <a{if $item.children} href="#" class="dropdown-toggle" data-toggle="dropdown"{else} href="{$item.fullurl}" target="{$item.targetwindow}"{/if}{if $item.attributes} {$item.attributes}{/if}>
