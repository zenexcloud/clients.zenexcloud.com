{if $captcha->isEnabled() && $captcha->isEnabledForForm($captchaForm)}
    <div class="text-center{if $containerClass}{$containerClass}{else} row justify-content-center{/if} captcha-cloudx-theme">
       ...
       {if $captcha->recaptcha->isEnabled() && !$captcha->recaptcha->isInvisible()}
        ...
        {elseif !$captcha->recaptcha->isEnabled()}
            <div class="col-md-12">
                <div id="default-captcha-domainchecker" class="{if $filename == 'domainchecker'}input-group input-group-box {/if}row pb-3">
